<div class="dlabnav">
    <div class="dlabnav-scroll">

        <ul class="metismenu" id="menu">
            <li><a class="" href="{{url('mimin/beranda')}}" aria-expanded="false">
                    <i class="fa fa-th"></i>
                    <span class="nav-text">Beranda</span>
                </a>
            </li>
            <li><a class="" href="{{url('mimin/permohonan')}}" aria-expanded="false">
                    <i class="fa fa-flag"></i>
                    <span class="nav-text">Permohonan</span>
                </a>
            </li>
            <li><a class="" href="{{url('mimin/pelayanan')}}" aria-expanded="false">
                    <i class="fa fa-info-circle"></i>
                    <span class="nav-text">Pelayanan</span>
                </a>
            </li>
            <li><a class="" href="laporan.html" aria-expanded="false">
                    <i class="fa fa-chart-area"></i>
                    <span class="nav-text">Laporan</span>
                </a>
            </li>
            <li><a class="" href="{{url('mimin/pengaturan')}}" aria-expanded="false">
                    <i class="fa fa-tools"></i>
                    <span class="nav-text">Pengaturan</span>
                </a>
            </li>
            <li><a class="" href="{{url('mimin/pesan')}}" aria-expanded="false">
                    <i class="fa fa-bell"></i>
                    <span class="nav-text">Pesan</span>
                </a>
            </li>

        </ul>
        <div class="plus-box">
            <p class="fs-14 font-w600 mb-2">SABER <br>memudahkan<br>Pelayanan,<br>Pajak Kendaraan <br></p>
            <p>Untuk Kalimantan Barat<br>lebih baik</p>
            <img src="images/SABERfinal.png" width="100%">
        </div>
        <div class="copyright">
            <p><strong>SABER</strong> © 2023 All Rights Reserved</p>
            <p class="fs-12">Made with <span class="heart"></span> by ACI</p>
        </div>
    </div>
</div>
